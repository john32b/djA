...
{	
	...
	// written to a file. Used in `scanNode()` 
	// DEV: Is Global, because it is build from a recursive function
	var _renderedTxt:Array<String>;
	...
	**/
	public function save():Bool
	{
		...
		scanNode(data);
		try{
			File.saveContent(pathFile, _renderedTxt.join('\n'));
		}catch (e:Any)
		{
		...
		for (l in lines)
		{
			...
			if (fc == "#" || fc == ";")
			{
				...
				continue;
									...
									// Get multiple values if there exist
	...
	**/
	...
	{
		setFieldFromAr(field.split('.'), v);
	}//---------------------------------------------------;
	
	/**
	   Full Field name e.g. "psx.controller.one"
	   @param	field
	   @return
	**/
	...
	**/
	...
	/**
	   ...
	   @param	v Value
	**/
	function setFieldFromAr(ar:Array<String>, v:String)
	{
		var o:Dynamic = data;
		var i = 0;
		while (i < ar.length - 1)
	...
	/**
	   Fills '_renderedTxt', so make sure to zero it out before calling
	...
	function scanNode(o:Dynamic, path:String = "")
	{
		...
		while (i < f.length)
		{
			...
			if (Std.is(v,String))
			{
				...
				if (com != null) for (c in com) _renderedTxt.push(c);
				_renderedTxt.push('$str $v');
				// No new line?
			}else{
